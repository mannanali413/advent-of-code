function rowDifference(row){
    let entries = row.split("\t").filter((entry) => {
        return entry !== " "
    }).map((entry) => {
    ...
    for(let i =0; i< entries.length; i++){
        ...
        for(let j = 0; j < entries.length; j++){
            let num1 = entries[i];
            let num2 = entries[j];
            if(num1 > num2 && parseInt(num1/num2, 10) === num1/num2 ){
                found = true;
    ...
    console.log('found the difference for',entries,'as', diff);
    return diff;
}
